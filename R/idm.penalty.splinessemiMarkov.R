...
#' @useDynLib HIDeM
idm.penalty.splinessemiMarkov<-function(b,fix0,size_V,size_spline,
                      ...
                      nlambda12,lambda12,alpha,penalty.factor,penalty,partialH){
  ...
  alphagrid01<-data.frame(alpha01=alpha[[1]],
                          lambda01=lambda01)
  alphagrid02<-data.frame(alpha02=alpha[[2]],
                          lambda02=lambda02)
  alphagrid12<-data.frame(alpha12=alpha[[3]],
                          lambda12=lambda12)
  
  lambdamerge<-data.frame(lambda)
  colnames(lambdamerge)<-c("lambda01","lambda02","lambda12")
  alphagrid<-merge(alphagrid01,lambdamerge,by=c("lambda01"))
  alphagrid<-merge(alphagrid02,alphagrid,by=c("lambda02"))
  alphagrid<-merge(alphagrid12,alphagrid,by=c("lambda12"))
  alpha<-alphagrid[,c("alpha01","alpha02","alpha12")]
                             ...
                             .errorhandling = "remove")%dopar%{
                               ...
                               while(converged==F & ite<=maxiter){
                                 ...
                                 if(ite==0){ # take value of loglik at initialisation
                                   ...
                                   fn.value<-idmlLikelihoodpenasemiMarkov(b=b,
                                                                npm=length(b),
                                                                npar=size_V,
                                                                bfix=bfix,
                                                                fix=fix0,
                                 ...
                                 res<-idmlLikelihoodpenasemiMarkov(b=b,
                                                         npm=length(b),
                                                         npar=size_V,
                                                         bfix=1,
                                                         fix=rep(0,size_V),
                                 ...
                                 if(res %in%c(-1e9,1e9) | res < fn.value){
                                   ...
                                   sears<-searpas(vw=vw,
                                                  ...
                                                  b=beta,
                                                  delta=delta,
                                                  funcpa=idmlLikelihoodpenasemiMarkov,
                               ...
                               .errorhandling = "remove")%dopar%{
                                 ...
                                 while(converged==F & ite<=maxiter){
                                   ...
                                   # derivative of loglik
                                     ...
                                     output<-derivasplinediagsemiMarkov(b=b,
                                                              npm=length(b),
                                                              npar=size_V,
                                                              bfix=bfix,
                                                              fix=fix0,
