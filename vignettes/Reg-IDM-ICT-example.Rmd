...
```{r setup,eval=F}
remotes::install_github("arianebercu/HIDeM")
```
...
- outer loop on penalty parameters noted $(a,\lambda_{01},\lambda_{02},\lambda_{12})$, and 
...
As we are in simulation setting we can evaluate its predicitive abilities using : The predictive ability is evaluated by the Mean Square Error on the probability for a healthy individual of becoming diseased at a time $u$ between 0 and t, 
$$\displaystyle{\widehat{MSEP}=\frac{1}{N}\sum_{i=1}^{N}(\widehat F_{01}(t_i) - \widetilde F_{01}(t_i))^2}
\mbox{ with }F_{01}(t)=\int_{0}^{t}e^{-A_{01}(u)-A_{02}(u)}\alpha_{01}(u)du,$$
where $N$ is the number of individuals, $t$ the time of first event between the illness, the death illness-free and the administrative censoring ($t$=min\{$T^{01},T^{02},18$\}), $\widehat F_{01}(t)$ the estimated probability by the fitted model and $\widetilde F_{01}(t)$ the respective true probability modeled in the simulation. 

We need to model a test replicate, the $\widehat{MSEP}$ is calculated by estimating the model with the variable selected in the regularized approach on the corresponding data train replicate. 
